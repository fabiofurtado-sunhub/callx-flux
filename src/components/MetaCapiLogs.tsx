...
interface MetaCapiLogsProps {
  leadId?: string;
  limit?: number;
}

export default function MetaCapiLogs({ leadId, limit = 10 }: MetaCapiLogsProps) {
  ...
  const fetchLogs = async () => {
    ...
    if (data) setLogs(data as CapiLog[]);
    setLoading(false);
  };
  ...
  useEffect(() => {
    ...
    const channel = supabase
      .channel('capi-logs-realtime')
      .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'meta_capi_logs' }, () => {
        fetchLogs();
      })
      .subscribe();
  ...
  if (logs.length === 0) {
    return (
      <p className="text-xs text-muted-foreground py-2">Nenhum evento CAPI registrado.</p>
    );
  }
    ...
    <div className="space-y-2">
      {logs.map(log => (
        ...
        >
          ...
          <Zap className="w-3 h-3 text-primary/40 shrink-0 mt-0.5" />
        </div>
      ))}
    </div>
  );
