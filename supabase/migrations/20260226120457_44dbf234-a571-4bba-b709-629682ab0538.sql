...
-- Update trigger function to also auto-send WhatsApp for core_ai leads
...
DECLARE
  ...
  _message text;
BEGIN
  IF NEW.status_funil = 'lead' AND NEW.envio_whatsapp_status = 'pendente' AND NEW.funil IN ('callx', 'core_ai') THEN
    
    IF NEW.funil = 'core_ai' THEN
      -- Fetch active template for core_ai lead stage
      SELECT conteudo INTO _template_content
      FROM public.message_templates
      WHERE funil = 'core_ai' AND etapa = 'lead' AND ativo = true
      ORDER BY ordem ASC
      LIMIT 1;
    ...
    ELSE
      -- CallX: use default behavior (no message_override, send-whatsapp uses built-in template)
      SELECT net.http_post(
        url := 'https://tqzrebkunvezpdeipamf.supabase.co/functions/v1/send-whatsapp',
        body := json_build_object(
          'lead_id', NEW.id,
